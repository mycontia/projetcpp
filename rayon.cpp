...
bool Quad::est_dans_surf(Vector3f v){
    ...
    return (((std::abs(dist_u) <= max_u) && (std::abs(dist_v) <= max_v))); // Le point est à l'intérieur du quad ou non
//LIB
}
...
answer Sphere::is_hit(Ray3f ray){
    answer reponse = {false, Vector3f(),Vector3f()}; //DEFAUT
    //calcul du discrimant du polynome du second degré (a*x^2 + b*x + c = 0) que l'on obtient en combinant 2 égalités sur comment s'écrit un point appartenant à un rayon et la condtion pour que ce point appartienne à la sphère 
    ...
    if (discriminent == 0) { //un seul point d'intersection
        reponse.hit = true;
        float t = - b / (2*a);
    ...
    if (discriminent >= 0) {
        reponse.norm = (reponse.pt_inter - origin_).normalise(); //le vecteur normal au plan de la sphère contenant le point d'intersection 
...
answer Cube::is_hit(Ray3f ray){
    ...
    for (size_t i = 0; i < q.size(); i++) {
        answer pt_inter = q[i].is_hit(ray); // stock si il a intersection entre le rayon et le quadrilatère et en cas de collision contient aussi les coordonnées du point le plus proche ((0,0,0) sinon) ainsi que la nomale à la surface où il y a collision ((0,0,0) sinon))
        if (pt_inter.hit) {
...
Ray3f Shape::reflect(Ray3f r, answer a) { 
    ...
    if(a.hit && (matter_.shininess_ !=0)){ //si le coefficient de brillande vaut 0, l'objet ne réfléchit pas le rayon
        ...
        reflet = Ray3f(a.pt_inter + a.norm * 0.001f, direction_reflet); // si avec les arrondis de calculs on a un pt_iter qui est légèrement décalé et à l'interieur de la shape, la réflection va donc avoir lieu sur la même shape donc on décale légèrement le point d'intersection pour compenser les erreurs de calculs sur les flottants
    }
    return reflet;
    }
...
void Scene::render(int x_taille, int y_taille, std::string fichier){
    ...
    try{
        ...
        if (prod_scal(h,w) != 0){ //on veut construire une boite en forme de parallélépipède rectangle donc avec des faces orthogonales
        ...
        while (SDL_PollEvent(&e) != 0) {
            ...
            if (e.type == SDL_KEYDOWN) {
                ...
                switch (e.key.keysym.sym) {
                    ...
                    case SDLK_z: source_.origin_.y_ -= vitesse; break; // Monte (Y diminue)
                    case SDLK_s: source_.origin_.y_ += vitesse; break; // Descend (Y augmente)
                    case SDLK_q: source_.origin_.x_ -= vitesse; break; // Gauche (X diminue)
                    case SDLK_d: source_.origin_.x_ += vitesse; break; // Droite (X augmente)
                    case SDLK_e: source_.origin_.z_ += vitesse; break; // Fond (Z augmente)
                    case SDLK_w: source_.origin_.z_ -= vitesse; break; // Vers nous (Z diminue)
                    
                    // Conservation des flèches pour la CAMÉRA
                    case SDLK_UP:    camera_.position_.y_ += vitesse; break; 
                    case SDLK_DOWN:  camera_.position_.y_ -= vitesse; break;
                    case SDLK_LEFT:  camera_.position_.x_ += vitesse; break;
                    case SDLK_RIGHT: camera_.position_.x_ -= vitesse; break;
                }
                image_finale = false; 
                ecran.clear(); 
            }
        ...
        if (!image_finale){
            ...
            for (int y = 0; y < y_taille; y++) {
                for (int x = 0; x < x_taille; x++) {
                    ...
                    Vector3f direction = (Vector3f(coord_x, coord_y, -2.0f) - camera_.position_).normalise();
                    Ray3f ray(camera_.position_, direction);
                    ...
                    for (size_t i =0; i< shapes_.size(); i++){
                        ...
                        if (rep.hit) {
                            float dist = (rep.pt_inter - camera_.position_).norme();
                            if (dist < dist_min) {
                                dist_min = dist;
